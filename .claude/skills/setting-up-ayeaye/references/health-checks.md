...
2. **The key has been switched off** — an `AYEAYE_TOKEN` override, or a
   daemon started in a state that skips the check. Restart it clean:
   `ayeaye service stop`, `ayeaye service start`, `ayeaye check`.

## The checks, one by one

**service** — *ayeaye starts when you log in.*
`FAILED`: the manager knows the unit and it is not running. `ayeaye service
status` prints the manager's own words; `ayeaye service repair` rewrites the
definition and restarts a running service. `skipped`: this machine has no user
...
Asserted in both directions, and only the second proves anything: a server
accepting every Host header passes the first half perfectly. `FAILED`'s detail
says which direction broke — a stranger being answered means the proxy
forwards unfiltered (tighten the proxy); a named host refused means
`AYEAYE_ALLOWED_HOSTS` and the proxy's idea of the name disagree. `skipped`:
no hosts configured, nothing claimed.

**https** — *the address the phone opens answers.*
A 401/403 refusal counts as `ok` — it proves the address reaches ayeaye,
which is all this check claims. `FAILED`: the front end itself did not answer
— certificate, proxy down, DNS. `unknown`: the bind is exposed but no host is
named in `AYEAYE_ALLOWED_HOSTS`, so there was no address to dial — name one,
or return to loopback.
...
`unknown`: cliban is on PATH and the app would not answer about it — the
daemon may predate cliban's arrival (restart it), or `AYEAYE_CLIBAN` points
somewhere stale. `skipped`: no cliban here, and the board tab simply stays
off.
