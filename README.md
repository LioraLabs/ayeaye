...
- **Token auth on every `/api/*` endpoint.** A shared secret comes from
  `VOICE_REMOTE_TOKEN` or, if unset, is generated with mode 0600 at
  `$XDG_STATE_HOME/voice-remote/token` on first run. Requests present it as
...
- **Host/Origin validation.** Requests whose `Host` header is not the bind
  ...
  `VOICE_REMOTE_ALLOWED_HOSTS` are rejected with a 403, which defeats DNS
...
1. Start `voice-remote` once; note the token
   (`cat ~/.local/state/voice-remote/token`).
...
- **`voice-remote`**: the web app and its API. Reads tmux, tails transcripts,
...
```sh
git clone https://github.com/LioraLabs/voice-remote ~/dev/voice-remote
cd ~/dev/voice-remote
./install.sh
```

The installer checks dependencies, writes one config file at
`~/.config/voice-remote/env`, creates the auth token, installs a systemd
user unit that runs `bin/voice-remote` straight from the clone, and prints
...
(`bin/voice-remote` reads the config file by itself). Re-running is safe:
an existing config file is never overwritten without asking.

Check it:

```sh
systemctl --user is-active voice-remote
journalctl --user -u voice-remote -f
curl -s -H "X-Voice-Token: $(cat ~/.local/state/voice-remote/token)" \
...
One env var. Point `VOICE_NTFY_URL` in `~/.config/voice-remote/env` at any
...
Everything lives in one file: `~/.config/voice-remote/env`, written by
...
| variable | default | |
|---|---|---|
| `VOICE_REMOTE_BIND` | `127.0.0.1` | bind address; see Security |
| `VOICE_REMOTE_PORT` | `8911` | |
| `VOICE_REMOTE_TOKEN` | generated | auth token; unset = generated 0600 in `$XDG_STATE_HOME/voice-remote/token` |
| `VOICE_REMOTE_ALLOWED_HOSTS` | unset | extra allowed `Host` values (your https front), comma separated |
| `VOICE_REMOTE_SHARE` | auto | dir holding `app.html`; defaults to the repo's `share/` |
| `VOICE_REMOTE_LINES` | `24` | pane lines in the terminal view |
...
your phone, so `voice-remote` can push when that happens.
...
Then point the service at it in `~/.config/voice-remote/env` and install
