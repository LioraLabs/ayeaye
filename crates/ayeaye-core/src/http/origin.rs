...
/// read is exempt and everything that could write is judged. The two header
/// arguments are the values as they arrived, untrimmed and in whatever case
/// they were sent; `None` means the header was absent. A header whose bytes
/// are not text is the caller's problem to render rather than to drop —
/// `None` is the *allowing* branch for `Origin`, so an unreadable one must
/// not arrive here as one.
pub fn gate(
    method: &str,
...
mod tests {
    ...
    fn a_cross_site_label_is_the_one_refusal() {
        assert_eq!(
            gate("POST", Some("cross-site"), None, &allowed()),
            Site::Cross
        );
    ...
    fn an_opaque_origin_is_refused() {
        for origin in ["null", "NULL", " null "] {
            assert_eq!(
                gate("POST", None, Some(origin), &allowed()),
                Site::Cross,
    ...
    fn a_request_that_names_no_origin_is_allowed() {
        assert_eq!(gate("POST", None, None, &allowed()), Site::Same);
        ...
        assert_eq!(gate("POST", None, Some(""), &allowed()), Site::Same);
        assert_eq!(gate("POST", None, Some("   "), &allowed()), Site::Same);
    ...
    fn an_origin_that_is_not_a_url_names_no_host() {
        ...
        assert_eq!(
            gate("POST", None, Some("//phone.local"), &allowed()),
            Site::Same
        );
    }
    ...
    fn an_unbalanced_ipv6_bracket_is_refused_even_if_the_allow_list_holds_it() {
        ...
        assert_eq!(
            gate("POST", None, Some("http://[::1"), &typo),
            Site::Cross,
            "an unbalanced bracket is where urlsplit raises"
        );
        assert_eq!(gate("POST", None, Some("http://::1]"), &typo), Site::Cross);
        ...
        assert_eq!(gate("POST", None, Some("http://[::1]"), &ok), Site::Same);
        assert_eq!(
            gate("POST", None, Some("http://[::1]:8912"), &ok),
            Site::Same
        );
    }
}
